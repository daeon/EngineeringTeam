...
Do not copy a Run Ledger into memory. A Run Ledger is task-scoped trace evidence. Use `references/memory-promotion.md` and `templates/memory-candidates.md` to promote only durable, reusable findings.
...
Promoted entries should include:

- Status: `current`, `needs-verification`, or `stale`
- Evidence/source paths
- Origin run, if available
- Last verified
